 ...
 * 获取权限验证配置信息
 */
// let getUrl = this.$http.getConfig
let getWxConfig = function getWxConfig() {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: `http://store.demo.ichebaoyang.com/wx/apiHandler.ashx?action=WxConfig&url=${window.navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/) ? encodeURIComponent(window.location.href.split('#')[0]) : encodeURIComponent(window.location.href)}`,
            type: "get",
            success(wxConfig) {
                resolve(wxConfig)
            },
